...
module.exports = {
    ...
    view: function ({ attrs }) {
        const session = controller.getCurrentSession(attrs.sessionId);
        const roundState = model.state.roundState;
        if (!session) {
            return;
        }

        return (
            <Body title='WOF Intro Game'>
                ...
                <div >
                    <div class="uk-flex uk-flex-bottom uk-flex-around uk-text-center uk-margin-remove uk-padding-remove">
                        {/* <button class="wof-btn wof-is-info wof-btn-medium wof-margin-small wof-size-large"
                            onclick={() => {
                    ...
                    <div class="wof-body wof-container">
                        <button class="wof-btn wof-is-primary wof-btn-medium wof-margin-small"
                            ...
                            {`Round #${roundState.currentRoundIndex + 2}: Reveal Puzzle`}
                        </button>
                    </div>
                </div>
            </Body>
